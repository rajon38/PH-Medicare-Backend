...
const initiatePayment = async (appointmentId: string, user: IRequestUser) => {
    ...
    if (appointmentData.status === AppointmentStatus.CANCELED) {
        throw new AppError(status.BAD_REQUEST, "Cannot initiate payment for cancelled appointment");
    }
            ...
            line_items: [
              {
                price_data: {
                  currency: 'bdt',
                    ...
                    unit_amount: appointmentData.doctor.appointmentFee * 120, // amount in cents
                },
                quantity: 1,
              },
            ],
